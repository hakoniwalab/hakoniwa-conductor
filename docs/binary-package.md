...
```bash
sha256sum -c hakoniwa-conductor-v1.0.0-linux-x86_64.zip.sha256
unzip hakoniwa-conductor-v1.0.0-linux-x86_64.zip
```

macOS:

```bash
shasum -a 256 -c hakoniwa-conductor-v1.0.0-macos-arm64.zip.sha256
unzip hakoniwa-conductor-v1.0.0-macos-arm64.zip
```
...
- `metadata/`: platform, pinned Foundation revisions, package inventory, and dependency inspection results;
...
The v1.0.0 ZIP names above are fixed. If a Recipe expects another name, correct the Recipe instead of renaming the ZIP.
